...
// ルート：　/collections/ ~
// 棋譜追加のページへ
router.get('/:collectionId/kifs/newKIF', authenticationEnsurer, csrfProtection, (req, res, next) => {
  Collection.findOne({
    where: {
...
// 棋譜 create
router.post('/:collectionId/kifs', authenticationEnsurer, csrfProtection, (req, res, next) => {
  const kifId = uuid.v4();
...
// 棋譜の編集ページへ
router.get('/:collectionId/kifs/:kifId/edit', authenticationEnsurer, csrfProtection, (req, res, next) => {
  Kif.findOne({
    where: { kifId: req.params.kifId }
  }).then((kif) => {
...
// edit で棋譜を編集・削除
router.post('/:collectionId/kifs/:kifId', authenticationEnsurer, csrfProtection, (req, res, next) => {
    ...
    Kif.findOne({
      ...
      if (functions.isMine(req, kif)) {
        kif.update({
          ...
          res.redirect('/collections/' + req.params.collectionId + /kifs/ + req.params.kifId);
        });
      } else {
